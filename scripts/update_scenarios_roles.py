...
scenario_files = [
    BASE / 'BotC Helper' / 'Resources' / 'sects_and_violets.json',
    BASE / 'BotC Helper' / 'Resources' / 'bad_moon_rising.json'
]
...
for scenario_path in scenario_files:
    ...
    for entry in data:
        eid = entry.get('id')
        # keep meta or entries without id as-is
            ...
            for rn in ['firstNightReminder', 'otherNightReminder']:
                if rn in entry and entry[rn]:
                    ...
                    if rn in base and isinstance(base[rn], str):
                        base[rn] = {'es': base[rn], 'en': roles_map[eid].get(rn, {}).get('en', '')}
                    elif rn not in base:
                        base[rn] = {'es': '', 'en': ''}
    ...
    out_path = scenario_path.with_suffix('.updated.json')
    ...
    out_path.rename(scenario_path)
    print(f'Replaced original {scenario_path.name} with updated file.')
